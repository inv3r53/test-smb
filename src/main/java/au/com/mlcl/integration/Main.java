...
public class Main {
	...
	private static void writeFile(DiskShare share,String fileName, String dirName, boolean append, Object data, String encoding) throws Exception {
        ...
        try {
            ...
            File f;
            if (append) {
                f = share.openFile(buildPath(dirName, fileName), EnumSet.of(AccessMask.GENERIC_ALL), fileAttributes, SMB2ShareAccess.ALL, SMB2CreateDisposition.FILE_OPEN_IF, createOptions);
            } else {
                f = share.openFile(buildPath(dirName, fileName), EnumSet.of(AccessMask.GENERIC_ALL), fileAttributes, SMB2ShareAccess.ALL, SMB2CreateDisposition.FILE_OVERWRITE_IF, createOptions);
            }
            ...
            if (data instanceof InputStream) {
                ...
                throw new RuntimeException("unsupported object type for file write: " + data.getClass() + ", supported types are InputStream, String or byte[]");
            }
            f.close();
            out = null;
        } catch (Exception e) {
